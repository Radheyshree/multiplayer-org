import { counterparty, isBounce, isOutside, learnZohoTemplate, mailProviderOf, parseUpdate, systemFromUrl, tagUpdate, zohoTicketUrl, type ExternalSystem } from './origin';
import { storage, storageReady, xyne } from './xyne';

/* ---- from lib/mailbridge.ts ------------------------------------------- */
/**
...
export async function loadMailThread(item: {
  ...
  let rows: ThreadEmailRow[];
  try {
    const { spaces } = await xyne();
    rows = (await spaces.email.listForConversations(
      [item.conversationId],
      item.channelId,
    )) as unknown as ThreadEmailRow[];
  } catch {
...
export function learnFrom(messages: Array<{ metadata?: unknown }>): void {
  for (const m of messages) {
    ...
    if (typeof url === 'string' && learnZohoTemplate(url)) void rememberOrigin(url);
  }
}
...
async function rememberOrigin(webUrl: string): Promise<void> {
  if (!storageReady) return;
  try {
    await storage
